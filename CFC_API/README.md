...
| Action |	Method Type |	Request | Description |
|---|---|---|---|
|`/api/user/signin`| `POST`|`{"user_id": String,"password":"string}`
|`/api/user/userid`| `Get`|
...
| `api/user/updateUserRecord` | `Post` | `{"id": String,"mobileno": String, "state": String, "city": String, "pincode": String, "address":  String }`
| `api/user/requestSchedule/:userId` | `Post ` | ` { "scheduleType": "Daily","scheduleStartDate": "10/29/2020","message": "",}` 
| `api/user/cancleScheduleRequest/:userId` | `Post ` | ` {  "requestId":"requstTd"}`
| `api/user/getFormDataQuestions/:userId` | `Post ` |
| `api/user/getrequestScheduleList/:userId` | `Post ` |
| `api/user/:userId` | `Post ` |`Key: data, value: {"name" : string","mobileno":mobile,"address":address,"state":state, "city":city, "pincode":pincode, "country":country, "officeCity":officeCity,"officeLocation":officeLocation, "floor":number, "zipcode":number, "locationId":locationId}`
| `api/user/deleteUserToken/:userId` | `Post ` |
| `api/user/getLocationConfiguration` | `Post ` |
| `api/user/generateOtp` | `Post ` | ` {"userId":userId}`
| `api/user/verifyOtp` | `Post ` | ` {"userId":userId}`
| `api/patient/updateDeviceToken` | `Post ` | ` {"userId":userId}`
